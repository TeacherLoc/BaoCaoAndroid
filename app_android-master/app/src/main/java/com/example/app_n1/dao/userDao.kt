...
fun calculateBMR(weight: Double, height: Double, age: Int, isMale: String): Double {
    return if (isMale == "Nam") {
        String.format("%.2f",(88.362 + (13.397 * weight) + (4.799 * height) - (5.677 * age))).toDouble()
    } else {
        String.format("%.2f",(447.593 + (9.247 * weight) + (3.098 * height) - (4.330 * age))).toDouble()
    }
}

fun calculateBMI(weight: Double, height: Double): Double {
    return String.format("%.2f",(weight / ((height).pow(2)))).toDouble()
}
